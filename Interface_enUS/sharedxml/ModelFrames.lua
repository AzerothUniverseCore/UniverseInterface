...
-- Returns the settings for the connected player's race/sex (evaluated dynamically on each call)
...
-- Returns the zoom offset based on the model (DressUp / Inspect)
...
function SharedXML_Model_OnLoad(self, maxZoom, minZoom, defaultRotation, onMouseUp)
	...
	-- SetFacing is the 3.3.5 API for orienting the model
...
function SharedXML_Model_OnMouseDown(model, button)
	if button == "LeftButton" or not button then
		-- Left click: rotation by drag
		model.mouseDown = true
		model.rotationCursorStart = GetCursorPosition()
	elseif button == "RightButton" then
		-- Right click: character panning + display of the visual cursor
...
function SharedXML_Model_OnUpdate(self, elapsedTime, rotationsPerSecond)
	...
	-- Panning (movement) right click
	elseif self.panning then
		local modelScale = self:GetModelScale()
		local cursorX, cursorY = GetCursorPosition()
		local scale = UIParent:GetEffectiveScale()
		ModelsPanningFrame:SetPoint("BOTTOMLEFT", cursorX / scale - 16, cursorY / scale - 16)	-- centers the visual cursor on the mouse

		local settings = self.settings or SelectSettings()

		if settings then
			local zoom = self.zoomLevel or self.minZoom
			zoom = 1 + zoom - self.minZoom	-- 1 at minimum zoom

			-- Transformation ratio: same cursor speed regardless of zoom
			...
			-- Update the reference cursor for the next frame
