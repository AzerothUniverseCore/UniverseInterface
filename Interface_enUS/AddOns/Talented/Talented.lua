-- FIX : Talented s'appuie normalement sur un addon compagnon "Talented_Loader"
-- (declare via "## LoadManagers: Talented_Loader AddonLoader" dans le .toc)
-- dont le seul role est de creer la table globale Talented AVANT que ce
-- fichier ne s'execute. Ce loader n'etant pas present/actif ici, _G.Talented
-- valait nil et TOUTES les lignes "Talented.xxx = ..." de ce fichier (et de
-- Options.lua/Ui.lua/Glyphs.lua, qui font tous "local Talented = _G.Talented"
-- de la meme facon) plantaient avec "attempt to index local 'Talented' (a
-- nil value)". On cree donc la table nous-memes si elle n'existe pas deja,
-- ce qui ne change rien si un Loader fonctionnel est present par ailleurs.
_G.Talented = _G.Talented or LibStub("AceAddon-3.0"):NewAddon("Talented", "AceEvent-3.0", "AceHook-3.0", "AceConsole-3.0")
...
do
	...
	-- PATCH AzerothUniverse : Data.lua/Exporters.lua sont volontairement absents de ce
	-- build (systeme de talents custom independant, seule l'UI Glyphes de Talented est
	-- utilisee ici). Talented.tabdata reste donc nil, et ces 3 lignes (qui ne servent qu'a
	-- alimenter le menu de selection d'arbre de talents de familier dans l'UI Talents
	-- classique de Talented, jamais ouverte dans ce build) faisaient planter l'addon des
	-- son chargement. On les protege pour ne les executer que si tabdata existe.
