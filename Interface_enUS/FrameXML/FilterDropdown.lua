...
function FilterDropDownSystem.SetUpDropDownLevel(dropdown, filterSystem, level)
	for filterIndex, filterInfo in ipairs(filterSystem.filters) do
		...
		elseif filterInfo.type == FilterComponent.Submenu and filterInfo.childrenInfo ~= nil then
			local checkable, set, isSet = filterInfo.checkable
			if checkable then
				set = function(_, _, _, value)
					filterInfo.set(value);
					if filterSystem.onUpdate then
						filterSystem.onUpdate();
					end
				end
				isSet = function() return filterInfo.isSet(filterInfo.filter); end;
			end
			FilterDropDownSystem.AddSubMenuButton(filterInfo.text, filterInfo.value, set, isSet, level, checkable);
...
function FilterDropDownSystem.AddSubMenuButton(text, value, set, isChecked, level, checkable)
	local subMenuInfo = {
		keepShownOnClick = true,
		hasArrow = true,
		notCheckable = not checkable,
		isNotRadio = not not checkable,
		checked = isChecked,
		func = set,
		text = text,
		value = value,
