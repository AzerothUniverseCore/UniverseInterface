...
function PKBT_CurrencyMixin:GetCurrencyType()
...
function PKBT_CurrencyListMixin:OnEvent(event, ...)
...
function PKBT_CurrencyListMixin:SetCurrencyAutoValueUpdateEnabled(state)
	if state and self:CanUpdateCurrencyValues() then
...
function PKBT_CurrencyListMixin:CanUpdateCurrencyValues()
	return not IsOnGlueScreen()
end

function PKBT_CurrencyListMixin:UpdateCurrencyValues()
	if self:CanUpdateCurrencyValues() then
		for index, currencyWidget in ipairs(self.activeCurrencies) do
			local currencyType = currencyWidget:GetCurrencyType()
			if currencyType and C_StorePublic.IsValidCurrencyType(currencyType) then
				local amount = C_StorePublic.GetBalance(currencyType)
				currencyWidget:SetValue(amount)
			end
		end
	end
	self.DIRTY = nil
...
function PKBT_CheckButtonBaseMixin:SetTextColor(r, g, b, a)
	self.ButtonText:SetTextColor(r, g, b, a)
end

function PKBT_CheckButtonBaseMixin:SetShadowColor(r, g, b, a)
	self.ButtonText:SetShadowColor(r, g, b, a)
end

function PKBT_CheckButtonBaseMixin:SetTextWidth(width)
	self.ButtonText:SetWidth(width)
	self:UpdateHitRect()
end

function PKBT_CheckButtonBaseMixin:SetMaxLines(numLines)
	self.ButtonText:SetMaxLines(numLines)
end

function PKBT_CheckButtonBaseMixin:SetFontObject(fontObject)
	self.ButtonText:SetFontObject(fontObject)
end

function PKBT_CheckButtonBaseMixin:GetHeightEx()
	return math.max(self:GetHeight(), self.ButtonText:GetHeight())
end

function PKBT_CheckButtonBaseMixin:UpdateHitRect()
	local yOffset = math.ceil((self:GetHeight() - self:GetHeightEx()) / 2) - 4
	self:SetHitRectInsets(0, -(5 + self.ButtonText:GetWidth()), yOffset, yOffset)
end
...
PKBT_CheckButtonHolderMixin = {}

function PKBT_CheckButtonHolderMixin:OnShow()
	self:UpdateRect()
end

function PKBT_CheckButtonHolderMixin:Enable()
	self.CheckButton:Enable()
end

function PKBT_CheckButtonHolderMixin:Disable()
	self.CheckButton:Disable()
end

function PKBT_CheckButtonHolderMixin:SetEnabled(enabled)
	self.CheckButton:SetEnabled(enabled)
end

function PKBT_CheckButtonHolderMixin:SetChecked(checked)
	self.CheckButton:SetChecked(checked)
end

function PKBT_CheckButtonHolderMixin:GetChecked()
	return self.CheckButton:GetChecked()
end

function PKBT_CheckButtonHolderMixin:SetText(text)
	self.CheckButton:SetText(text)
	self:UpdateRect()
end

function PKBT_CheckButtonHolderMixin:UpdateRect()
	local width =  self.CheckButton:GetWidth() + 5 + self.CheckButton.ButtonText:GetWidth()
	local height = math.ceil(self.CheckButton:GetHeightEx())
	self:SetSize(width, height)
end

function PKBT_CheckButtonHolderMixin:GetText()
	self.CheckButton:GetText()
end

function PKBT_CheckButtonHolderMixin:SetTextColor(r, g, b, a)
	self.CheckButton:SetTextColor(r, g, b, a)
end

function PKBT_CheckButtonHolderMixin:SetShadowColor(r, g, b, a)
	self.CheckButton:SetShadowColor(r, g, b, a)
end

function PKBT_CheckButtonHolderMixin:SetTextWidth(width)
	self.CheckButton:SetTextWidth(width)
end

function PKBT_CheckButtonHolderMixin:SetMaxLines(numLines)
	self.CheckButton:SetMaxLines(numLines)
end

function PKBT_CheckButtonHolderMixin:SetFontObject(fontObject)
	self.CheckButton:SetFontObject(fontObject)
end

function PKBT_CheckButtonHolderMixin:SetOnCheckedCallback(onChecked)
	self.CheckButton.OnChecked = onChecked
end
...
function PKBT_RibbonMixin:SetFormattedText(fmt, text)
	self.Text:SetFormattedText(fmt, text)
end

function PKBT_RibbonMixin:SetFontObject(fontObject)
...
function PKBT_UIPanelScrollBarMixin:UpdateThumbSize(width, height, minValue, maxValue)
	...
	elseif maxValue ~= 0 then
		...
		self.ThumbTexture:SetHeight(math.min(math.max(27, visibleHeight), height * 0.45))
	elseif height then
...
function PKBT_EditBoxCodeMixin:OnTextChanged(userInput)
	...
	if type(self.OnTextValueChangedPre) == "function" then
		self.OnTextValueChangedPre(self, text, userInput)
	end
...
function PKBT_EditBoxCodeMixin:ClearText(clearFocus)
	self:SetText("")

	if type(self.OnTextValueChanged) == "function" then
		self.OnTextValueChanged(self, "", true)
	end
...
function PKBT_EditBoxMixin:OnLoad()
	...
	self.ClearButton.Icon:SetAtlas("PKBT-Icon-Clear", true)
end

function PKBT_EditBoxMixin:OnEditFocusGained()
	if self.__clearButtonEnabled then
		self.ClearButton:Show()
	end
end

function PKBT_EditBoxMixin:OnEditFocusLost()
	if self.__clearButtonEnabled then
		if self.__originalText == "" then
			self.ClearButton:Hide()
		end
	end
end

function PKBT_EditBoxMixin:OnTextValueChangedPre(text, userInput)
	if self.__clearButtonEnabled then
		self.ClearButton:SetShown(text ~= "" and self:HasFocus())
	end
end

function PKBT_EditBoxMixin:SetClearButtonEnabled(state)
	self.__clearButtonEnabled = not not state

	if self.__clearButtonEnabled then
		self:SetTextInsets(9, 29, 4, 0)
		self.ClearButton:SetShown(self.__originalText ~= "" and self:HasFocus())
	else
		self:SetTextInsets(9, 9, 4, 0)
		self.ClearButton:Hide()
	end
end
...
PKBT_EditBoxFilterMixin = CreateFromMixins(PKBT_EditBoxMixin)

function PKBT_EditBoxFilterMixin:OnHide()
	self:StopOnCharFilterDelay()
end

function PKBT_EditBoxFilterMixin:OnEnterPressed()
	self:ClearFocus()
	self:ApplyFilter()
end

function PKBT_EditBoxFilterMixin:OnEditFocusLost()
	self:HighlightText(0, 0)
	self:ApplyFilter()
end

function PKBT_EditBoxFilterMixin:OnEscapePressed()
	self:SetText(self.value or "")
	PKBT_EditBoxMixin.OnEscapePressed(self)
end

function PKBT_EditBoxFilterMixin:OnApplyFilter(value)
end

function PKBT_EditBoxFilterMixin:ApplyFilter()
	local newValue = self:GetText()
	if self.value ~= newValue and not (self.value == nil and newValue == "") then
		self:OnApplyFilter(newValue)
		self.value = newValue
	end
end

function PKBT_EditBoxFilterMixin:ToggleOnCharFilter(state, filterUpdateDelay)
	if state then
...
function PKBT_EditBoxFilterMixin:OnTextValueChanged(value, userInput)
	if userInput then
...
function PKBT_EditBoxFilterMixin:OnUpdate(elapsed)
...
function PKBT_EditBoxFilterMixin:StopOnCharFilterDelay()
	self:SetScript("OnUpdate", nil)
...
function PKBT_SimpleHTMLAsFontStringMixin:GetShadowOffset()
	self:GetRegions():GetShadowOffset()
end

function PKBT_SimpleHTMLAsFontStringMixin:SetShadowOffset(offsetX, offsetY)
	self:GetRegions():SetShadowOffset(offsetX, offsetY)
end

function PKBT_SimpleHTMLAsFontStringMixin:GetShadowColor()
	self:GetRegions():GetShadowColor()
end

function PKBT_SimpleHTMLAsFontStringMixin:SetShadowColor(r, g, b, a)
	self:GetRegions():SetShadowColor(r, g, b, a)
end
...
function PKBT_DropdownMenuMixin:Enable()
	self.Button:Enable()
	self.Text:SetTextColor(HIGHLIGHT_FONT_COLOR:GetRGB())
	self.Icon:SetDesaturated(false)
end

function PKBT_DropdownMenuMixin:Disable()
	self.Button:Disable()
	self.Text:SetTextColor(DISABLED_FONT_COLOR:GetRGB())
	self.Icon:SetDesaturated(true)
end

function PKBT_DropdownMenuMixin:SetEnabled(state)
	if state then
		self:Enable()
	else
		self:Disable()
...
if not IsOnGlueScreen() then
	local IsCreatureDataCached = IsCreatureDataCached
	...
	ModelLoadHandler:SetScript("OnUpdate", function(self, elapsed)
		...
		while modelData and index <= queueLen do
			local modelType = modelData[REQUEST_FIELD.MODEL_TYPE]
			local modelID = modelData[REQUEST_FIELD.MODEL_ID]
			...
			if link then
				...
				if TOOLTIP:IsShown() then
					...
					if modelData[REQUEST_FIELD.SUCCESS_CALLBACK] then
						local success, err = pcall(modelData[REQUEST_FIELD.SUCCESS_CALLBACK], modelData[REQUEST_FIELD.MODEL_OBJ], modelType, modelID)
						if not success then
							geterrorhandler()(err)
						end
					end
				...
				table.remove(self.modelQueue, index)
				if modelData[REQUEST_FIELD.ERROR_CALLBACK] then
					local success, err = pcall(modelData[REQUEST_FIELD.ERROR_CALLBACK], modelData[REQUEST_FIELD.MODEL_OBJ], modelType, modelID)
					if not success then
						geterrorhandler()(err)
					end
				end
	...
	function ModelLoadHandler:FireCacheCallback(cacheRequest, success)
		self:UnqueueModel(cacheRequest[REQUEST_FIELD.MODEL_OBJ])

		local callback
		if success then
			callback = cacheRequest[REQUEST_FIELD.SUCCESS_CALLBACK]
		else
			callback = cacheRequest[REQUEST_FIELD.ERROR_CALLBACK]
		end

		if callback then
			local _success, err = pcall(callback, cacheRequest[REQUEST_FIELD.MODEL_OBJ], cacheRequest[REQUEST_FIELD.MODEL_TYPE], cacheRequest[REQUEST_FIELD.MODEL_ID])
	...
	function ModelLoadHandler:IsInvalidOrCacheLoaded(modelType, modelID)
		...
		if creatureID then
			return IsCreatureDataCached(creatureID)
		end
	...
	function ModelLoadHandler:QueueModel(model, modelType, modelID, successCallback, errorCallback)
		...
		if creatureID then
			...
			if IsCreatureDataCached(creatureID) then
			...
			if cacheRequestObj then
				if cacheRequestObj[REQUEST_FIELD.MODEL_TYPE] == modelType
				or cacheRequestObj[REQUEST_FIELD.MODEL_ID] == modelID
				then
					-- same model, update callbacks
					cacheRequestObj[REQUEST_FIELD.SUCCESS_CALLBACK] = successCallback
					cacheRequestObj[REQUEST_FIELD.ERROR_CALLBACK] = errorCallback
				else
					-- remove old request
			...
			if not cacheRequestObj then
				local cacheRequest = {
					[REQUEST_FIELD.MODEL_OBJ]			= model,
					[REQUEST_FIELD.MODEL_TYPE]			= modelType,
					[REQUEST_FIELD.MODEL_ID]			= modelID,
					[REQUEST_FIELD.SUCCESS_CALLBACK]	= successCallback,
					[REQUEST_FIELD.ERROR_CALLBACK]		= errorCallback,
				}
		...
		do
			...
			for index, cacheRequest in ipairs(self.modelQueue) do
				if cacheRequest[REQUEST_FIELD.MODEL_OBJ] == model
				and (successCallback == nil or cacheRequest[REQUEST_FIELD.SUCCESS_CALLBACK] == successCallback)
				then
					cacheRequest[REQUEST_FIELD.MODEL_TYPE]			= modelType
					cacheRequest[REQUEST_FIELD.MODEL_ID]			= modelID
					cacheRequest[REQUEST_FIELD.SUCCESS_CALLBACK]	= successCallback
					cacheRequest[REQUEST_FIELD.ERROR_CALLBACK]		= errorCallback
					found = true
					break
				end
			end
	...
	function ModelLoadHandler:UnqueueModel(model)
		...
		if cacheRequest then
			local creatureID = cacheRequest[REQUEST_FIELD.MODEL_ID]
			if CACHE_REQUESTS[creatureID] then
				local index = tIndexOf(CACHE_REQUESTS[creatureID], cacheRequest)
				if index then
					table.remove(CACHE_REQUESTS[creatureID], index)
					if #CACHE_REQUESTS[creatureID] == 0 then
						CACHE_REQUESTS[creatureID] = nil
					end
				end
			end
		...
		do
			...
			for index, modelRequest in ipairs(self.modelQueue) do
				if modelRequest[REQUEST_FIELD.MODEL_OBJ] == model then
					table.remove(self.modelQueue, index)
					removed = true
					break
				end
		...
		if self.awaitItemDataID then
			if itemID == self.awaitItemDataID then
				...
				if success then
					...
					self.reloadTimer = C_Timer.NewTimer(0, function()
						self.needsReload = true
						self:UpdateModelPreset()
					end)
				end
			end
		elseif self.awaitItemSetIDs then
			local index = tContains(self.awaitItemSetIDs, itemID)
			if index then
				table.remove(self.awaitItemSetIDs, index)

				if #self.awaitItemSetIDs == 0 then
					self:UnregisterCustomEvent(event)
				end
...
function PKBT_ModelMixin:SetM2Model(model, facing, posOffsetX, posOffsetY, posOffsetZ, modelScale, createCamera)
...
function PKBT_ModelMixin:SetCustomizationDressUp(itemLink, facing, posOffsetX, posOffsetY, posOffsetZ, undress, preserveFacting, preservePosition)
	local modelType = self.modelType
	self:PreserveCameraSettings(preserveFacting, preservePosition)
	self:ResetFull(preserveFacting, preservePosition)
	self.modelType = Enum.ModelType.Customization
	self.modelID = GetItemInfoInstant(itemLink)
	self.facing = facing
	self.basePositionOffsetX = posOffsetX
	self.basePositionOffsetY = posOffsetY
...
function PKBT_ModelMixin:SetModelAuto(modelType, modelID, facing, posOffsetX, posOffsetY, posOffsetZ, ...)
	...
	elseif modelType == Enum.ModelType.Customization then
		self:SetCustomizationDressUp(modelID, facing, posOffsetX, posOffsetY, posOffsetZ, ...)
...
function PKBT_ModelMixin:UpdateModelPreset()
	...
	elseif self.modelType == Enum.ModelType.Item
	or self.modelType == Enum.ModelType.Customization
	then
		self:SetUnit("player")
		if self.undress then
			self:Undress()
	...
	elseif self.modelType == Enum.ModelType.ItemSet then
		...
		for index, itemLink in ipairs(self.modelID) do
			local itemID = C_Item.GetItemIDFromString(itemLink)
			if not C_Item.IsItemInfoLoaded(itemID)
			and (not self.awaitItemSetIDs or not tContains(self.awaitItemSetIDs, itemID))
			then
				if not self.awaitItemSetIDs then
					self.awaitItemSetIDs = {}
				end
				table.insert(self.awaitItemSetIDs, itemID)
				self:RegisterEvent("ITEM_DATA_LOAD_RESULT")
				C_Item.RequestServerCache(itemID)
			end
			self:TryOn(itemLink)
	...
	elseif self.modelType == Enum.ModelType.M2 then
		if self.createCamera then
			self:SetModel([[Character\Human\Male\HumanMale.mdx]])
			self:SetCamera(1)
		else
			self:SetCamera(0)
		end
...
function PKBT_DressUpBaseMixin:SetModel(modelType, modelID, undress, preserveFacting, preservePosition)
	...
	or modelType == Enum.ModelType.Customization
	then
...
function PKBT_DressUpBaseMixin:DressUpItemLink(link)
	...
	elseif linkType == Enum.DressUpLinkType.Customization then
		return self:SetModel(Enum.ModelType.Customization, id)
...
PKBT_MountAbilityOverlayMixin = {}

function PKBT_MountAbilityOverlayMixin:OnLoad()
	self.abilitiesPool = CreateFramePool("Button", self, self:GetAttribute("abilityButtonTemplate"))
	self.basePoint = "BOTTOMRIGHT"
	self.baseOffsetX = 0
	self.baseOffsetY = 0
	self.directionPoint = "TOPRIGHT"
	self.directionOffsetX = 0
	self.directionOffsetY = -6
end

function PKBT_MountAbilityOverlayMixin:SetDrawPoint(point, offsetX, offsetY, directionPoint, directionOffsetX, directionOffsetY)
	self.basePoint = point
	self.baseOffsetX = offsetX
	self.baseOffsetY = offsetY
	self.directionPoint = directionPoint
	self.directionOffsetX = directionOffsetX
	self.directionOffsetY = directionOffsetY
end

function PKBT_MountAbilityOverlayMixin:ClearItemAbilities(releasePool)
	if releasePool then
...
function PKBT_MountAbilityOverlayMixin:SetItemAbilities(item)
	self.abilitiesPool:ReleaseAll()
	...
	if abilities and #abilities > 0 then
		...
		for _, abilityID in ipairs(abilities) do
			...
			if not lastButton then
				...
				button:SetPoint(self.basePoint, lastButton, self.directionPoint, self.directionOffsetX, self.directionOffsetY)
			end
			lastButton = button
		end

		local buttonWidth, buttonHeight = lastButton:GetSize()
		self:SetSize(buttonWidth * #abilities, buttonHeight * #abilities)
