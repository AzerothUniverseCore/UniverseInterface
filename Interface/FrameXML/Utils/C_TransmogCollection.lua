...
local function SearchAndFilterIllusions()
	...
	for illusionIndex, illusions in ipairs(ILLUSIONS) do
		...
		for _, enchantIndex in ipairs(illusions) do
			...
			if enchantData then
				local productID, price, originalPrice, currencyType, categoryID, subCategoryID = C_StorePublic.GetRolledItemInfoByHash(enchantData.hash)
...
local function BuildIllusions()
	...
	for index, enchantData in ipairs(COLLECTION_ENCHANTDATA) do
		...
		if enchantData.enchId and enchantData.spellID then
			...
			C_SpellBook.FilterOutSpellLearn(enchantData.spellID, enchantData.name)
		end
	end
end
...
frame:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_LOGIN" then
		...
		C_FactionManager.RegisterCallback(UpdatePlayerFactionID, true, true);
...
function C_TransmogCollection.GetAppearanceSourceDrops(itemID)
	local instance, encounter, difficulties = C_EncounterJournal.GetItemSourceDrops(itemID)
	if instance then
		return {
			{
