...
local function FilteredPetJornal()
	...
	for i = 1, #COLLECTION_PETDATA do
		...
		local productID, price, originalPrice, currencyType, categoryID, subCategoryID = C_StorePublic.GetRolledItemInfoByHash(data.hash)
...
local function GetPetInfo(infoTable, value)
	...
	if petInfo then
		...
		local productID, price, originalPrice, currencyType, categoryID, subCategoryID = C_StorePublic.GetRolledItemInfoByHash(petInfo.hash)
		if not productID then
...
function C_PetJournal.GetPetTypeByItemID(itemID)
	if type(itemID) ~= "number" then
		error("Usage: C_PetJournal.GetPetTypeByItemID(itemID)", 2)
	end

	local petInfo = PET_INFO_BY_ITEM_ID[itemID]
	if petInfo then
		return petInfo.subCategoryID
	end
end
