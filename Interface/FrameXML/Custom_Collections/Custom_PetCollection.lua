...
function PetJournal_UpdatePetDisplay()
	if PetJournal.selectedPetID then
		...
		PetJournal.PetDisplay.ModelScene:SetCreatureModel(creatureID, MODELFRAME_DEFAULT_ROTATION);
...
function PetJournalBuyButton_OnClick()
	...
	if PetJournal.selectedPetID then
		local name, _, icon, _, _, sourceType, _, _, hash, _, itemID, currencyType, price, productID = C_PetJournal.GetPetInfoByPetID(PetJournal.selectedPetID);
			...
			elseif currencyType and currencyType ~= 0 then
				if currencyType == Enum.Store.CurrencyType.Bonus then
					...
					if _productID then
						...
						GMError(string.format("[PetJournal] attempt to open store for collection item with hash `%s` for `%s`", hash, _productID))
					end
				elseif C_StorePublic.IsValidCurrencyType(currencyType) then
					...
					GMError(string.format("[PetJournal] attempt to open store for collection item with currencyType `%s`", currencyType))
				end
			end
		end
	end
