...
function MountJournal_UpdateMountDisplay(forceSceneChange)
	if MountJournal.selectedMountID then
		local creatureName, spellID, icon, active, sourceType, isFavorite, isFactionSpecific, faction, isCollected, mountID, creatureID, itemID, currency, price, _, priceText, descriptionText, holidayText, disablePlayerMountPreview = C_MountJournal.GetMountInfoByID(MountJournal.selectedMountID);
		...
		if MountJournal.MountDisplay.lastDisplayed ~= itemID or forceSceneChange then
			...
			MountJournal.MountDisplay.ModelScene.spellID = spellID
			MountJournal.MountDisplay.ModelScene.disablePlayerMountPreview = disablePlayerMountPreview
...
function MountJournal_Select(index)
	-- PATCH Collection : garde-fou (index nil observe en jeu, ex. rafraichissement
	-- de la liste avant toute selection) pour eviter l'erreur
	-- "Usage: C_MountJournal.GetDisplayedMountInfo(displayIndex)".
...
function MountJournalMountButton_UseMount(mountID)
	-- PATCH Collection : garde-fou contre un mountID nil/invalide (evite
	-- "Usage: C_MountJournal.GetMountCompanionIndex(mountID)").
	if type(mountID) ~= "number" and type(mountID) ~= "string" then
		return;
	end
...
function MountListDragButton_OnClick(self, button)
	...
	elseif IsModifiedClick("CHATLINK") then
		...
		-- PATCH Collection : garde-fou contre un parent.mountID nil/invalide.
		if type(parent.mountID) == "number" or type(parent.mountID) == "string" then
...
function MountJournalBuyButton_OnClick(self, button)
	...
	if MountJournal.selectedMountID then
		local creatureName, _, icon, _, sourceType, _, _, _, _, hash, _, itemID, currencyType, price, productID = C_MountJournal.GetMountInfoByID(MountJournal.selectedMountID);
		...
		elseif currencyType and currencyType ~= 0 then
			if currencyType == Enum.Store.CurrencyType.Bonus then
				local _productID, _price, _originalPrice, _currencyType, _categoryID, _subCategoryID = C_StorePublic.GetRolledItemInfoByHash(hash)
				if _productID then
					local categoryIndex, subCategoryIndex = C_StoreSecure.GetVirtualCategoryByRemoteID(_currencyType, _categoryID, _subCategoryID)
					StoreFrame:ShowProductCategory(productID, categoryIndex, subCategoryIndex)
				else
					GMError(string.format("[MountJournal] attempt to open store for collection item with hash `%s` for `%s`", hash, _productID))
				end
			elseif C_StorePublic.IsValidCurrencyType(currencyType) then
				local categoryIndex, subCategoryIndex = C_StoreSecure.GetVirtualCategoryByRemoteID(currencyType, 0, 0)
				StoreFrame:ShowProductCategory(productID, categoryIndex, subCategoryIndex)
			else
				GMError(string.format("[MountJournal] attempt to open store for collection item with currencyType `%s`", currencyType))
			end
		end
	end
end
...
function ListScrollFrame_OnShow(self)
	...
	MountJournal.LeftInset.Bg:SetVertexColor(1, 1, 1, 1)
end

function CategoryScrollFrame_OnShow(self)
	MountJournal_FilterToggle(false)
	MountJournal.LeftInset.Bg:SetVertexColor(1, 0, 0, 0.9)
end
...
function MountJournal_ModelScene_OnShow(self)
	self:ClearModel()
	self:SetPosition(0, 0, 0)

	if self.creatureID then
		if not self.disablePlayerMountPreview and self.spellID then
			if IsCreatureDataCached(self.creatureID) then
				self:UnregisterEvent("CREATURE_DATA_LOAD_RESULT")
			else
				self:RegisterEvent("CREATURE_DATA_LOAD_RESULT")
				RequestLoadCreatureByID(self.creatureID)
			end
			self:SetUnitMount("player", self.spellID)
		else
			self:UnregisterEvent("CREATURE_DATA_LOAD_RESULT")
			self:SetCreatureModel(self.creatureID, MODELFRAME_DEFAULT_ROTATION)
		end
	end
...
function MountJournal_ModelScene_OnHide(self)
	self:UnregisterEvent("CREATURE_DATA_LOAD_RESULT")
end

function MountJournal_ModelScene_OnEvent(self, event, ...)
	if event == "CREATURE_DATA_LOAD_RESULT" then
		local creatureID, success = ...
		if self.creatureID == creatureID then
			if success and not self.disablePlayerMountPreview then
				MountJournal_ModelScene_OnShow(self)
			end
			self:UnregisterEvent("CREATURE_DATA_LOAD_RESULT")
		end
	end
