...
SlashCmdList["BMAH"] = function(msg)
  ...
  elseif msg == "fill" then
    ...
    -- show usage
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r "..L.USAGE)
  end
end
----------------------------------------------------------------------------
-- Localization (frFR / enUS, falls back to enUS for any other client locale)
----------------------------------------------------------------------------
local LOCALES = {
  frFR = {
    USAGE               = "Usage : /bmah [fill|flush] (GM UNIQUEMENT)",
    CONFIRM_BID          = "Etes-vous sur de vouloir encherir %dg sur |cff00ff00%s|r ?",
    SELLER_LABEL         = "Vendeur :\n%s",
    TIME_LEFT_LABEL       = "Temps restant : %s",
    COL_LEVEL            = "Niv",
    BID_BUTTON           = "Encherir",
    MARKET_CLOSED        = "Marche actuellement ferme",
    NO_ITEMS             = "Aucun objet n'est actuellement en vente. Revenez plus tard !",
    SELECT_ITEM_FIRST     = "Veuillez d'abord selectionner un objet pour encherir.",
    INVALID_SELECTION     = "Selection invalide.",
    NO_HOT_ITEM          = "Aucun objet vedette disponible pour le moment.",
    HOT_ITEM_LOST         = "Donnees de l'objet vedette perdues, veuillez rouvrir la fenetre.",
    UNKNOWN_ITEM         = "Objet inconnu",
    UNKNOWN_TYPE         = "Type inconnu",
    FRAME_NOT_FOUND       = "BlackMarketFrame introuvable au chargement de l'addon.",
    ONSHOW_NOT_FOUND      = "BlackMarketFrame introuvable pour le hook OnShow !",
    TITLE                = "Hôtel des ventes\nau marché noir",
  },
  enUS = {
    USAGE               = "Usage: /bmah [fill|flush] (GM ONLY)",
    CONFIRM_BID          = "Are you sure you want to bid %dg on |cff00ff00%s|r?",
    SELLER_LABEL         = "Seller:\n%s",
    TIME_LEFT_LABEL       = "Time Left: %s",
    COL_LEVEL            = "Lvl",
    BID_BUTTON           = "Bid",
    MARKET_CLOSED        = "Market Currently Closed",
    NO_ITEMS             = "There are no items currently on market. Check back later!",
    SELECT_ITEM_FIRST     = "Please select an item to bid on first.",
    INVALID_SELECTION     = "Invalid selection.",
    NO_HOT_ITEM          = "No hot item available to bid on right now.",
    HOT_ITEM_LOST         = "Hot item data lost, please reopen the UI.",
    UNKNOWN_ITEM         = "Unknown Item",
    UNKNOWN_TYPE         = "Unknown Type",
    FRAME_NOT_FOUND       = "BlackMarketFrame not found on ADDON_LOADED.",
    ONSHOW_NOT_FOUND      = "BlackMarketFrame not found for OnShow hook!",
    TITLE                = "Black Market\nAuction House",
  },
}
...
StaticPopupDialogs["BMAH_CONFIRM_BID"] = {
  -- FIX: StaticPopup on this 3.3.5a client only supports `text` as a plain
  -- string auto-formatted with text_arg1/text_arg2 (the function-callback form
  -- is a much later Blizzard addition and does not exist here). Using a
  -- function crashed StaticPopup_Show silently before the popup ever
  -- displayed, which is why nothing happened when bidding. L is already
  -- resolved above, so this stays a plain localized string.
  text         = L.CONFIRM_BID,
  button1      = ACCEPT,
  button2      = CANCEL,
  hideOnEscape = 1,
  ...
  OnAccept = function(self, data)
    local bidMsg = ("BMAH_BID;%d;%d"):format(data.itemId, data.amount)
    SendChatMessage(bidMsg, "WHISPER", nil, UnitName("player"))
    MoneyInputFrame_SetCopper(BlackMarketBidPrice, 0)
  end,
}

-- FIX: BlackMarketUI.xml declares <OnLoad function="BlackMarketFrame_OnLoad"/> and
-- <OnShow function="BlackMarketFrame_OnShow"/>, but neither function existed anywhere in
-- this addon -- referencing an undefined global from XML does not error, it just makes
-- the script a silent no-op. Defining them for real so the XML hooks aren't dead refs.
...
local BIDOK           = "BMAH_BIDOK"

-- FIX: the server has no reliable way to read a player's client locale on
-- this core, so the client sends its own GetLocale() along with every
-- BMAH_REQ whisper (server parses "BMAH_REQ;frFR" / "BMAH_REQ;enUS"). This
-- is what lets bmah_server.lua reply with localized broadcast/mail text.
local function BMAH_SendReq()
  SendChatMessage(REQ .. ";" .. GetLocale(), "WHISPER", nil, UnitName("player"))
end

local rows = {}
local serverMaxRowId

-- FIX: the server used to crash trying to resolve an item's icon name
-- (tpl:GetIcon() does not exist on this core), so it no longer sends one.
-- Icons are now resolved here instead, using the real client API
-- GetItemIcon(itemId), which does not depend on the server at all. Falls
-- back to whatever icon name the server DID manage to send (if any, for
-- forward-compat) and finally to a generic question-mark icon.
local function BMAH_ResolveIcon(itemId, iconNameFallback)
  local clientIcon = itemId and GetItemIcon(itemId)
  if clientIcon then
    return clientIcon
  end
  if iconNameFallback and iconNameFallback ~= "" then
    return "Interface\\Icons\\" .. iconNameFallback
  end
  return "Interface\\Icons\\INV_Misc_QuestionMark"
end

-- FIX: the file used to create this frame TWICE in a row (the first copy
-- registered events but never got :SetScript("OnEvent", ...), so it just sat
-- there doing nothing -- harmless but confusing). Down to a single frame now.
...
f:SetScript("OnEvent", function(self, event, ...)
  ...
  elseif event == "CHAT_MSG_ADDON" then
    ...
    -- “OPEN” from your server-side NPC
    if prefix == UI_OPEN_PREFIX and msg == "OPEN" then
      BlackMarketFrame:Show()
      return
    end

    -- FIX: bid confirmation used to be detected by pattern-matching the
    -- English text of the CHAT_MSG_SYSTEM broadcast ("Your bid of %dg has
    -- been accepted"). That broke the moment the server message got
    -- localized to French. The server now also sends a dedicated,
    -- language-independent addon message ("BMAH_BIDOK") right after a
    -- successful bid -- use that instead to trigger the refresh.
    if prefix == BIDOK then
    ...
    -- FIX: removed a dead branch here that compared prefix == ADDON_PREFIX
    -- ("BMAH"). The server always sends the real prefix as "BMAH_DATA" /
    -- "BMAH_DONE" (see DATA/DONE below), so "BMAH" never matched and this
    -- code never ran. The actual DATA/DONE handling lives in the fallback
    -- block right after this function's if/elseif chain (it compares
    -- prefix == DATA / DONE directly, which does match).
  end
  ...
  if prefix == DATA then
    ...
    table.insert(rows, {
      ...
      id       = tonumber(rowIdStr), -- this row's real blackmarketauctionhouse.id
      _raw     = message,
    })
  ...
  elseif prefix == DONE then
    ...
    BlackMarketUI_UpdateList(rows)
    -- FIX: serverMaxRowId is the auction's real database id, not an array
    -- position. rows[serverMaxRowId] only happened to work by coincidence
    -- on a freshly-filled table (ids 1,2,3.. lining up with insertion
    -- order); it breaks the moment rows get flushed/refilled and ids stop
    -- matching array position. Search for the matching row by id instead.
    local hotRow
    hotItemIndex = nil
    for i, r in ipairs(rows) do
      if r.id == serverMaxRowId then
        hotRow = r
        hotItemIndex = i
        break
      end
    ...
    if hotRow then
      -- Icon
      local iconPath = BMAH_ResolveIcon(hotRow.itemId, hotRow.icon)
      ...
      if BlackMarketFrameHotItemName then
        BlackMarketFrameHotItemName:SetText(hotRow.itemName or L.UNKNOWN_ITEM)
      end

      -- Subclass
      if BlackMarketFrameHotItemSubclass then
        BlackMarketFrameHotItemSubclass:SetText((hotRow.itemType or L.UNKNOWN_TYPE))
      end

      -- Seller
      if BlackMarketFrameSellerLine then
        BlackMarketFrameSellerLine:SetText(L.SELLER_LABEL:format(hotRow.owner or "?"))
      end

      -- Time left
      if BlackMarketFrameTimeLeftLine then
        BlackMarketFrameTimeLeftLine:SetText(L.TIME_LEFT_LABEL:format(hotRow.timeLeft or "?"))
      end
    end
...
function BlackMarketUI_UpdateList(data)
  ...
  for i = 1, #buttons do
    ...
    if record then
      if btn.Icon then
        btn.Icon:SetTexture(BMAH_ResolveIcon(record.itemId, record.icon))
...
init:SetScript("OnEvent", function(self, event, addon)
  ...
  -- FIX: the "Black Market / Auction House" wood-sign title was hardcoded in
  -- XML (text="Black Market|nAuction House") with no parentKey, so it stayed
  -- English no matter the client locale. Gave it parentKey="TitleText" in
  -- BlackMarketUI.xml so it can be set here from the locale table.
  if BlackMarketSignFrame and BlackMarketSignFrame.TitleText then
    BlackMarketSignFrame.TitleText:SetText(L.TITLE)
  end
  ...
  if not f then
    DEFAULT_CHAT_FRAME:AddMessage(L.FRAME_NOT_FOUND)
    return
  end
  ...
  colLevel.Name:SetText(L.COL_LEVEL)
...
init:SetScript("OnEvent", function(self, event)
  ...
  secondBid:SetText(L.BID_BUTTON)
  ...
  sellerLine:SetText(L.MARKET_CLOSED)  -- default
  ...
  timeLeftLine:SetText(L.MARKET_CLOSED)  -- default
  ...
  nameText:SetText(L.MARKET_CLOSED)
  ...
  subclassText:SetText(L.MARKET_CLOSED)
  ...
  do
    ...
    if frame then
      ...
      DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[BMAH UI]|r "..L.ONSHOW_NOT_FOUND)
    end
  end
...
local function ClearBidInputsAndRefresh()
  ...
  if hotItemIndex then
    ...
    if hotRow then
      ...
      BlackMarketFrameSellerLine:SetText(L.SELLER_LABEL:format(hotRow.owner))
      BlackMarketFrameTimeLeftLine:SetText(L.TIME_LEFT_LABEL:format(hotRow.timeLeft))
    end
  end
end
...
function BlackMarketBid_OnClick(self)
  ...
  if not selectedIndex then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r "..L.SELECT_ITEM_FIRST)
    return
  end
  ...
  if not record then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r "..L.INVALID_SELECTION)
    return
  end
...
function BlackMarketHotBid_OnClick(self)
  if not hotItemIndex then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r "..L.NO_HOT_ITEM)
    return
  end
  local record = rows[hotItemIndex]
  if not record then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r "..L.HOT_ITEM_LOST)
    return
  end
  ...
  -- trigger confirmation popup:
  -- FIX: was sending record.itemId (the world item entry). The server's
  -- HOTBID_REQ handler filters "WHERE id = %d" on the auction's real row id,
  -- not the item entry, so this bid could never match a row. Send record.id.
  StaticPopup_Show(
    "BMAH_CONFIRM_BID",
    bidGold,
    record.itemName,
    { itemId = record.id, amount = bidGold }
  )
end
