...
-- ============================================================
-- HABILLAGE "GRIMOIRE" (theme parchemin, v1)
-- ============================================================
--
-- PATCH Azeroth Universe : passe une teinte parchemin/dore sur le
-- cadre existant plutot que de changer les fichiers de texture --
-- aucun rendu possible cote outil pour verifier un remplacement
-- d'atlas pixel par pixel, donc on reste sur du teintage de backdrop
-- (bgFile/edgeFile INCHANGES, juste recolores), qui ne peut jamais
-- produire de texture noire/manquante. A ajuster en jeu si besoin.

-- Alpha remis a 0 : depuis l'ajout du fond mountjournal-bg (texture
-- reelle, avec ses propres zones semi-transparentes sur les bords),
-- garder ce fond de backdrop teinte en dessous produisait une ombre
-- inconstante par endroits. Seule la bordure (couleur suivante) reste
-- visible pour encadrer le nouveau fond.
local GRIMOIRE_BG_COLOR =
    { 0.16, 0.11, 0.06, 0 }

local GRIMOIRE_BORDER_COLOR =
    { 0.72, 0.58, 0.27, 1 }

local GRIMOIRE_SLOT_BG_COLOR =
    { 0.10, 0.07, 0.04, 0.88 }

local GRIMOIRE_SLOT_BORDER_COLOR =
    { 0.62, 0.49, 0.24, 1 }

local GRIMOIRE_TEXT_COLOR =
    { 0.92, 0.80, 0.48 }

local GRIMOIRE_TEXT_COLOR_DIM =
    { 0.78, 0.68, 0.50 }


function PetBattleUI_Team_Skin()

    local frame =
        PetBattleUI_TeamFrame

    if frame and frame.SetBackdropColor then

        frame:SetBackdropColor(
            unpack(GRIMOIRE_BG_COLOR)
        )

        frame:SetBackdropBorderColor(
            unpack(GRIMOIRE_BORDER_COLOR)
        )
    ...
    local title =
        _G["PetBattleUI_TeamFrameTitle"]

    if title then
        title:SetTextColor(
            unpack(GRIMOIRE_TEXT_COLOR)
        )
    end


    local subtitle =
        _G["PetBattleUI_TeamFrameSubtitle"]

    if subtitle then
        subtitle:SetTextColor(
            unpack(GRIMOIRE_TEXT_COLOR_DIM)
        )
    end


    local subtitle2 =
        _G["PetBattleUI_TeamFrameSubtitle2"]
    ...
    for i = 1, TEAM_MAX_SLOTS do
        ...
        local slot =
            _G["PetBattleUI_TeamSlot" .. i]

        if slot and slot.SetBackdropColor then

            slot:SetBackdropColor(
                unpack(GRIMOIRE_SLOT_BG_COLOR)
            )

            slot:SetBackdropBorderColor(
                unpack(GRIMOIRE_SLOT_BORDER_COLOR)
            )
        ...
        local number =
            _G["PetBattleUI_TeamSlot" .. i .. "Number"]
        ...
        local name =
            _G["PetBattleUI_TeamSlot" .. i .. "Name"]
...
local function GetCompanionsWindow()
    ...
    -- Azeroth Universe : confirme via /fstack en jeu -- l'onglet
    -- "Compagnons" est en realite un onglet du Grimoire (SpellBookFrame),
    -- et le panneau qui liste les familiers s'appelle
    -- SpellBookCompanionsFrame (PAS PetJournal, ni PetPaperDollFrameCompanionFrame).
    -- Priorite absolue : c'est la fenetre reellement utilisee par ce client.

    if SpellBookCompanionsFrame then

        return SpellBookCompanionsFrame:GetParent()
            or SpellBookFrame

    end

    -- Repli : fenetre Collections de l'addon DragonUI, si jamais installe
    -- a la place du Grimoire natif.
    ...
    -- Repli : journal de Collections façon retail (si jamais present).

    if PetJournal then

        return CollectionsJournal
            or PetJournal:GetParent()

    end

    -- Repli : nom de frame stock 3.3.5 "historique".
...
function PetBattleUI_Team_OnReceiveDrag(slotButton)
    ...
    if dragSourceSlot then
        ...
        if sourceButton.Icon then
            ...
            sourceButton.Icon:SetTexCoord(0, 1, 0, 1)
        ...
        if targetButton.Icon then
            ...
            targetButton.Icon:SetTexCoord(0, 1, 0, 1)
...
-- Azeroth Universe : confirme via /fstack en jeu -- l'onglet
-- "Compagnons" est un onglet du Grimoire (SpellBookFrame), et le
-- panneau qui liste les familiers s'appelle SpellBookCompanionsFrame.
-- Ni PetJournal ni PetPaperDollFrameCompanionFrame n'existent dans ce
-- fork, d'ou l'absence totale de synchro avant ce correctif.
--
-- IMPORTANT (2e correctif) : SpellBookCompanionsFrame est en realite
-- un panneau PARTAGE, reutilise tel quel pour Métiers/Montures/
-- Compagnons -- confirme via /fstack : la meme pile de frames
-- (SpellBookCompanionsFrame > SpellBookPageNavigationFrame >
-- SpellBookFrame) apparait quel que soit l'onglet actif, seul le
-- CONTENU affiche change. Il ne se Hide() donc jamais quand on
-- change d'onglet, et se baser sur son OnShow/OnHide faisait
-- apparaitre l'Équipe de mascottes meme sur l'onglet Montures.
--
-- IMPORTANT (3e correctif) : la premiere tentative se basait sur
-- "SpellBookFrame.selectedTab" (convention Blizzard standard
-- PanelTemplates_SetTab), mais ce fork ne pose visiblement pas cette
-- propriete (elle reste nil) -- resultat : la condition etait
-- TOUJOURS fausse et l'equipe ne s'affichait plus du tout, meme sur
-- Compagnons. On ne devine donc plus l'etat interne : on le suit
-- nous-memes via le clic reel sur chaque onglet, ce qui est fiable
-- quelle que soit l'implementation interne du fork. Compagnons =
-- onglet 4 (Grimoire=1, Métiers=2, Montures=3, Compagnons=4 --
-- confirme via /fstack sur SpellBookFrameTabButton3 et
-- SpellBookFrameTabButton4).

local SPELLBOOK_COMPANIONS_TAB_ID = 4

-- Onglet actuellement selectionne, suivi manuellement via les clics
-- (voir plus bas). nil tant qu'aucun clic n'a encore eu lieu.
local currentSpellBookTab = nil


-- Repli utilise UNIQUEMENT au tout premier OnShow de la session (avant
-- le moindre clic) : sur un bouton d'onglet Blizzard standard
-- (PanelTemplates_TabButtonTemplate), l'onglet actif est desactive
-- (IsEnabled() == false) pour empecher de le recliquer -- c'est ce qui
-- lui donne son aspect "enfonce". Si ce fork suit cette convention
-- visuelle, ça permet de detecter l'onglet deja ouvert (fenetre
-- rouverte sur le dernier onglet utilise) sans dependre d'une
-- propriete interne devinee.
local function GuessActiveSpellBookTab()

    for i = 1, 4 do

        local tab = _G["SpellBookFrameTabButton" .. i]

        if tab and tab.IsEnabled and not tab:IsEnabled() then
...
local function UpdatePetBattleTeamVisibility_SpellBook()

    if currentSpellBookTab == nil then

        currentSpellBookTab = GuessActiveSpellBookTab()

    end

    if SpellBookFrame
        and SpellBookFrame:IsShown()
        and currentSpellBookTab == SPELLBOOK_COMPANIONS_TAB_ID then
...
if SpellBookFrame then
    ...
    SpellBookFrame:HookScript(
        "OnShow",
        UpdatePetBattleTeamVisibility_SpellBook
    )
    ...
    -- Changer d'onglet (Grimoire/Métiers/Montures/Compagnons) sans
    -- fermer la fenetre doit aussi mettre a jour la visibilite de
    -- l'equipe. On enregistre nous-memes l'onglet clique : c'est la
    -- source de verite, on ne depend plus d'une propriete devinee.
...
-- ============================================================
--
-- Repli additionnel (upstream FINAL) pour les clients qui utilisent
-- l'addon DragonUI a la place du Grimoire natif. Ne remplace PAS les
-- blocs ci-dessus : SpellBookCompanionsFrame reste prioritaire car
-- c'est la fenetre confirmee reellement utilisee sur ce fork.
...
local function SetupSingleActionButton(
    ...
    -- ====================================================
    -- PLAQUE DECORATIVE (fond) DERRIERE L'ICONE D'ACTION
    -- ====================================================
    ...
    if not button.Plate then
        ...
        -- Fichier STOCK Blizzard (deja present dans le client), teinte
        -- doree -- jamais de fichier personnalise.
        button.Plate:SetTexture(
            "Interface\\Buttons\\UI-Quickslot2"
        )

        button.Plate:SetVertexColor(
            1.0, 0.85, 0.35
        )

        button.Plate:SetPoint(
            "CENTER",
            button,
            "CENTER",
            0,
