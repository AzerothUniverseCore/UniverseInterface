...
function EncounterJournal_DisplayCreature(self)
	...
	-- PATCH v3 : utilise la vraie API native SetCreatureModel (fournie par
	-- PKBT_ModelMixin, cf SharedUIPanelPKBTTemplates.lua) au lieu du
	-- contournement EJ_SetCreatureModel -> Model:SetModel(chemin) qui ne
	-- recuperait jamais la variation de texture (modele blanc).
	EncounterJournal.encounter.info.model:SetCreatureModel(self.displayInfo, MODELFRAME_DEFAULT_ROTATION)
...
function EncounterJournal_SetLootButton(item)
	...
	if ( name ) then
		...
		local itemName, itemLink, quality = EJ_GetItemInfo(link or itemID)
