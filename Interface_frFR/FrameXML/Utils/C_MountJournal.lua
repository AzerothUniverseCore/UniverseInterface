...
local realmFilterData = {
	[-101] --[[ E_REALM_ID.SOULSEEKER absent d'Universe ]]	= 0x1,
	[E_REALM_ID.SCOURGE]	= 0x2,
	[E_REALM_ID.ALGALON]	= 0x4,
	[E_REALM_ID.SIRUS]		= 0x8,
	[-102] --[[ E_REALM_ID.NEVEREST absent d'Universe ]]	= 0x10,
};
...
function FilteredMountJornal()
	...
	if CATEGORY_FILTER then
		...
		for i = 1, #COLLECTION_MOUNTDATA do
			local mountInfo = COLLECTION_MOUNTDATA[i];
			local productID, price, originalPrice, currencyType, categoryID, subCategoryID = C_StorePublic.GetRolledItemInfoByHash(mountInfo.hash)
...
function C_MountJournal.GetMountAbilitiesByItemID(itemID)
	if type(itemID) ~= "number" then
		error("Usage: C_MountJournal.GetMountAbilitiesByItemID(itemID)", 2)
	end

	local mountInfo = MOUNT_INFO_BY_ITEM_ID[itemID]
	if mountInfo then
		return mountInfo.specialAbilities
	end
end
...
function C_MountJournal.GetMountFactionMaskByItemID(itemID)
	if type(itemID) ~= "number" then
		error("Usage: C_MountJournal.GetMountFactionMaskByItemID(itemID)", 2)
	end

	local mountInfo = MOUNT_INFO_BY_ITEM_ID[itemID]
	if mountInfo then
		return mountInfo.factionSide or 0
	end
end
