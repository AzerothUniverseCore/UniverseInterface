-- ============================================================
--  QuestMap - Furioz - Noa –  WoW 3.3.5a
-- ============================================================
...
function QuestMapFrame_OnLoad(self)
	-- FIX (Aurora) : l'icone "XP" (autre systeme/addon, ancre a une position ecran fixe et
	-- independante de QuestMapFrame) passait par-dessus le cadre lui-meme, pas seulement la
	-- carte a l'interieur - relever seulement WorldMapDetailFrame/Button/BlobFrame (la carte)
	-- ne suffisait pas puisque c'est le FOND/BORDURE de QuestMapFrame qui etait recouvert.
	-- Le classic WorldMapFrame utilise deja FULLSCREEN_DIALOG pour sa couche POI (cf. plus bas
	-- dans QuestMapFrame_DetachWorldMap) - on aligne donc tout le cadre sur la meme Strata.
	self:SetFrameStrata("FULLSCREEN_DIALOG");

	-- Le bouton fermer herite (PortraitFrameTemplate, offset x=4 y=5) et
	-- QuestMapMaximizeButton (offset x=-16 y=4) debordaient tous les deux au-dessus du cadre
	-- visible de QuestMapFrame ("hors de l'UI", faute d'une barre de titre habillee comme sur
	-- un PortraitFrameTemplate classique). On les replace tous les deux a l'interieur du cadre,
	-- cote a cote, une bonne fois pour toutes ici plutot que dans le XML.
	if self.CloseButton then
		self.CloseButton:ClearAllPoints();
		self.CloseButton:SetPoint("TOPRIGHT", self, "TOPRIGHT", 4, 4);
	end
	if QuestMapMaximizeButton then
		QuestMapMaximizeButton:ClearAllPoints();
		QuestMapMaximizeButton:SetPoint("TOPRIGHT", self, "TOPRIGHT", -19, 4);
	end
...
function QuestMapFrame_AttachWorldMap()
	...
	-- FIX (Aurora) : une fois la carte reparentee dans QuestMapFrame, d'autres overlays
	-- (ex. l'icone "XP") qui restent ancres plus haut dans la hierarchie continuaient de
	-- s'afficher par-dessus. Un simple FrameLevel plus eleve ne suffit pas si cet overlay
	-- est dans une FrameStrata plus haute que QuestMapFrame (la Strata prime toujours sur le
	-- Level) : on releve donc les deux, en sauvegardant les valeurs d'origine (restaurees
	-- dans QuestMapFrame_DetachWorldMap ci-dessous, meme principe que QM_savedWMBOnUpdate).
	if not QM_savedMapFrameLevels then
		QM_savedMapFrameLevels = {
			detail       = WorldMapDetailFrame:GetFrameLevel(),
			button       = WorldMapButton:GetFrameLevel(),
			blob         = WorldMapBlobFrame:GetFrameLevel(),
			poi          = WorldMapPOIFrame:GetFrameLevel(),
			detailStrata = WorldMapDetailFrame:GetFrameStrata(),
			buttonStrata = WorldMapButton:GetFrameStrata(),
			blobStrata   = WorldMapBlobFrame:GetFrameStrata(),
			poiStrata    = WorldMapPOIFrame:GetFrameStrata(),
		};
	end
	local scLevel = sc:GetFrameLevel();
	-- FIX (Aurora, suite) : "HIGH" ne suffisait pas face a l'overlay XP (un autre systeme/UI,
	-- donc probablement ancre a une Strata encore plus haute). On monte au niveau juste en
	-- dessous de TOOLTIP pour etre au-dessus de tout le reste sans passer devant les tooltips.
	WorldMapDetailFrame:SetFrameStrata("FULLSCREEN_DIALOG");
	WorldMapButton:SetFrameStrata("FULLSCREEN_DIALOG");
	WorldMapBlobFrame:SetFrameStrata("FULLSCREEN_DIALOG");
	WorldMapDetailFrame:SetFrameLevel(scLevel + 1);
	WorldMapButton:SetFrameLevel(scLevel + 2);
	WorldMapBlobFrame:SetFrameLevel(scLevel + 3);
...
function QuestMapFrame_DetachWorldMap()
	...
	-- FIX (Aurora, suite) : on restaure FrameLevel ET FrameStrata d'origine sauvegardes dans
	-- QuestMapFrame_AttachWorldMap, pour ne pas laisser une valeur elevee "fuiter" dans la
	-- hierarchie de la WorldMapFrame classique une fois la carte redetachee.
	if QM_savedMapFrameLevels then
		WorldMapDetailFrame:SetFrameStrata(QM_savedMapFrameLevels.detailStrata);
		WorldMapButton:SetFrameStrata(QM_savedMapFrameLevels.buttonStrata);
		WorldMapBlobFrame:SetFrameStrata(QM_savedMapFrameLevels.blobStrata);
		WorldMapPOIFrame:SetFrameStrata(QM_savedMapFrameLevels.poiStrata);
		WorldMapDetailFrame:SetFrameLevel(QM_savedMapFrameLevels.detail);
		WorldMapButton:SetFrameLevel(QM_savedMapFrameLevels.button);
		WorldMapBlobFrame:SetFrameLevel(QM_savedMapFrameLevels.blob);
		WorldMapPOIFrame:SetFrameLevel(QM_savedMapFrameLevels.poi);
		QM_savedMapFrameLevels = nil;
	end
...
-- FIX (Aurora) : QuestMapFrame est un frame custom, pas connu du systeme de fermeture par
-- Echap natif (contrairement a WorldMapFrame, deja gere par le moteur/UIPanelWindows). On
-- l'ajoute a UISpecialFrames (UIParent.lua), le mecanisme standard Blizzard qui fait qu'un
-- appui sur Echap masque automatiquement tout frame dont le nom y figure et qui est affiche.
